		...
		<? include "../pages/conexion_bd.php"; ?>
			...
			<ul class="nav nav-tabs">
				<li <? if($_GET["ronda"]=="todos" || ($_GET["ronda"]=="" &&  $_GET["apostante"]=="")){ echo 'class="active"';}?>>
					<a href="./cuartos.php?ronda=todos">Todos</a>
				</li>
				<li <? if($_GET["ronda"]=="ida"){ echo 'class="active"';}?>>
					<a href="./cuartos.php?ronda=ida">Ida</a>
				</li>
				<li <? if($_GET["ronda"]=="vuelta" ){ echo 'class="active"';}?>>
					<a href="./cuartos.php?ronda=vuelta">Vuelta</a>
				</li>
			</ul>
	...
	if($_GET["ronda"]!=""){
		if($_GET["ronda"]=="ida"){
			...
			$query='SELECT p.id, e.nombre as local, f.nombre as visitante, p.fecha, p.hora, p.apostante, p.resultado, e.escudo as escLocal, f.escudo as escVisit 
					...
					WHERE p.local=e.id and p.visitante=f.id and p.fase="CUARTOS"
					ORDER BY p.fecha, p.hora,p.id asc
					LIMIT 4';
		}else if($_GET["ronda"]=="vuelta"){
		...
		echo '<h4>Encuentros de '.$_GET["apostanteNom"].' </h4>';
			$query='SELECT p.id, e.nombre as local, f.nombre as visitante, p.fecha, p.hora, p.apostante, p.resultado, e.escudo as escLocal, f.escudo as escVisit 
					FROM partidos p, equipos e, equipos f 
					WHERE p.local=e.id and p.visitante=f.id and p.apostante='.$_GET["apostante"].' and p.fase="CUARTOS"
					ORDER BY p.fecha, p.hora,p.id asc';
					$result=mysql_query($query);
	}
}else{
...
<div class="tabbable tabs-below pull-right">
  <ul class="nav nav-tabs">
  	<?   	
		//Ejecutamos la sentencia SQL
		$result=mysql_query("select nombre, id from apostantes where id in (6,7,2,4) order by nombre asc");		
		while ($row=mysql_fetch_array($result)) {
			if($_GET["apostante"]==$row["id"]){
				echo '<li class="active"><a href="./cuartos.php?apostante='.$row["id"].'&apostanteNom='.$row["nombre"].'">'.$row["nombre"].'</a>';
			}else{						
		  		echo '<li><a href="./cuartos.php?apostante='.$row["id"].'&apostanteNom='.$row["nombre"].'">'.$row["nombre"].'</a>';
			}
		}mysql_free_result($result);
	?>
  </ul>
